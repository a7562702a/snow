...
public class ReplySQL extends Global { 
	...
	public ReplySQL() {//리플생성자
	 ...
	 public void dbReplyInsert(String w, String c, int sabun) {
		 try {
			 msg="insert into cb_reply values(cb_reply_seq.nextval, ?, ?,? )" ;
	...
	public ArrayList<ReplyDTO> dbReplySelect(int  Rdata){ //사번조건
		 ...
		 try {
		  ...
		  sb.append("select rownum rn, g.sabun, r.num, r.writer,r.content from cb_list g ");
		  sb.append(" inner join cb_reply r  ");
	 ...
	 public void dbReplyDelete(int rnum) { //리플한건삭제
		 try {
			 msg="delete from cb_reply where num = " + rnum ;
			 ST=CN.createStatement();
	 ...
	 public void dbReplyUpdate(ReplyDTO rdto) {//댓글수정
		 try {
			 msg="update cb_reply set writer=?, content=?  where num=?" ;
			 PST=CN.prepareStatement(msg);
	 ...
	 public int dbReplyCount(int Rdata) {
		 int count=0;
		 try {
			 msg="select  count(*) as cnt from cb_reply where sabun = " + Rdata ;
			 ST=CN.createStatement();
	 ...
	 public ArrayList<ReplyDTO> dbRelpySelect(int Rdata, int start, int end){ 
		 ArrayList<ReplyDTO> list=new ArrayList<ReplyDTO>();
		 try {
			 String x="select rownum rn,  r.* from  cb_reply r where sabun = " + Rdata ;
